...
class MailChannel(BaseChannel):
    ...
    def get_message(self, notifiable, notification):
        if hasattr(notification, 'to_email'):
            message = notification.to_email(notifiable)
            if not (isinstance(message, EmailMessage) or isinstance(message, EmailMultiAlternatives)):
                raise Exception("The to_email method should be return instance of EmailMessage or EmailMultiAlternatives")
            return message
        raise Exception("Notification is missing to_email method.")
